...
def coco_keypoint():
    ...
    for one_xml in train_xml_list:
        ...
        x = 0 if box_xmin - int(img_x * 0.14) < 0 else box_xmin - int(img_x * 0.14)
        y = 0 if box_ymin - int(img_y * 0.14) < 0 else box_ymin - int(img_y * 0.14)
        w = (box_xmax + int(img_x * 0.14)) - x
        h = (box_ymax + int(img_y * 0.14)) - y
