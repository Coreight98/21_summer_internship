...
img_path = '/home/crescom01/PycharmProjects/handbone_key_point/BA_label_point/image/train' + '/'
val_img_path = '/home/crescom01/PycharmProjects/handbone_key_point/BA_label_point/image/val' + '/'
...
xml_path = '/home/crescom01/PycharmProjects/handbone_key_point/json_new/train' + '/'
...
val_xml_list = sorted(os.listdir(val_xml_path))
...
def coco_keypoint():
    ...
    train_xml_list = xml_list
    # val_xml_list = val_xml_list
    ...
    for one_xml in train_xml_list:
        ...
        x_min = 0 if box_xmin - int(img_x * 0.2) < 0 else box_xmin - int(img_x * 0.2)
        y_min = 0 if box_ymin - int(img_y * 0.2) < 0 else box_ymin - int(img_y * 0.2)

        x_max = img_x if box_xmax + int(img_x * 0.2) > img_x else box_xmax + int(img_x * 0.2)
        y_max = img_y if box_ymax + int(img_y * 0.2) > img_y else box_ymax + int(img_y * 0.2)

        # print(temp_xml_list)
        train_y_dict['annotations'].append({'num_keypoints':num_keypoint, 'keypoints': [x_min,y_min,2,x_max,y_max,2],
                                            'image_id':id_cnt, 'category_id':1,
                                            'bbox':[0,0,img_x, img_y],
    ...
    for one_xml in val_xml_list:
        xml_dict = {}
        with open(val_xml_path + one_xml, 'r') as file:
            lines = file.readlines()
            for one_line in lines:
                # print(one_line)
                if '<name>' in one_line:
        ...
        img_name = one_xml.replace('.xml', '.jpg')
        img = cv2.imread(val_img_path + img_name)
        img_y, img_x = img.shape[0], img.shape[1]
        id_cnt += 1
        ann_id += 1

        val_y_dict['images'].append(
                {'file_name': str(id_cnt) + '.jpg', 'id': id_cnt, 'height': img_y, 'width': img_x})
        num_keypoint = len(point_list)
        ...
        # print(temp_xml_list)
        val_y_dict['annotations'].append({'num_keypoints': num_keypoint, 'keypoints': [x_min, y_min, 2, x_max, y_max, 2],
                                          'image_id': id_cnt, 'category_id': 1,
                                          'bbox': [0, 0, img_x, img_y],
                                          'area': img_x * img_y, 'iscrowd': 0,
                                          'id': ann_id})
        ...
        one_img_path = val_img_path + one_img_name
